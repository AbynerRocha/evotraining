...
import { AlertDialog, Menu, useDisclose } from 'native-base';
...
export default function EditWorkout() {
  ...
  const removeExerciseDialog = useDisclose()
  const refRemoveExerciseDialog = useRef(null)
  ...
  /> : (
    <View className='h-screen w-screen'>
            ...
            return (
              <Menu
                ...
                trigger={(triggerProps => {
                  ...
                  >
                    <Pressable className='pr-2 py-2 pl-1' onPress={() => {
                      setExerciseSelected(data.item)
                      removeExerciseDialog.onOpen()
                    }}>
      ...
      <AlertDialog
        leastDestructiveRef={refRemoveExerciseDialog}
        isOpen={removeExerciseDialog.isOpen}
        onClose={() => {
          removeExerciseDialog.onClose()
          setExerciseSelected(undefined)
        }}
        size='lg'
      >
        <AlertDialog.Content>
          ...
          <AlertDialog.Header><Text className='font-semibold'>Remover {exerciseSelected?.name}?</Text></AlertDialog.Header>
          <AlertDialog.Body>
            <Text>
              Você realmente quer remover este exercício?
            </Text>
          </AlertDialog.Body>
          <AlertDialog.Footer className='space-x-2'>
            <Button size='sm' onPress={handleRemoveExercise}>Sim</Button>
            <Button size='sm' color='red' onPress={() => {
              removeExerciseDialog.onClose()
              setExerciseSelected(undefined)
            }}>Não</Button>
          </AlertDialog.Footer>
