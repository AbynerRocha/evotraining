...
import { Dialog, DialogClose, DialogContent, DialogFooter, DialogTrigger } from '@/components/lib/ui/modal'
...
export default function CreateExercise() {
  ...
  const { register, handleSubmit, formState: { errors }, setError } = useForm<Fields>()
  ...
  function handleDropdownSelect(selected: { _id: string, name: string }) {
  ...
  function handleCreate(data: Fields) {
  ...
  return (
    <div className='flex flex-1 flex-col mt-4'>
      <h1 className='text-3xl font-medium mb-4'>Criar novo exercicio {exerciseName !== '' && `'${exerciseName}'`}</h1>
      <form
        method='POST'
      ...
      >
        <div className='flex flex-col space-y-1'>
          ...
          <input
            ...
            onChange={(v) => setExerciseName(v.target.value)}
            id='name'
        ...
        <div className='flex flex-col space-y-2 w-56'>
          ...
          <div className='flex flex-row w-60 h-fit space-x-2'>
            {muscles.map((muscle, idx) => (
              <div 
                key={idx} 
                className='h-8 w-fit border border-gray-300 p-3 flex flex-row items-center justify-between cursor-pointer whitespace-nowrap rounded-xl hover:text-gray-50 hover:bg-blue-700 hover:transition-all hover:ease-linear duration-200'
        ...
        >
          {isLoading ? <ReactLoading type='spin' height={24} width={24} color='white' /> : 'Criar'}
        </button>
...
/*
{showDropdown && <Animated.div
            className='relative top-2 h-fit w-full bg-gray-100 rounded-md border border-gray-300 shadow-sm shadow-black/30 flex flex-col justify-center'
            style={{ ...springsDropdown }}
          >
